...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const cScoreVal = document.getElementById('cScoreVal');
  ...
  function calculateLoanMl() {
    if (!applicantIncome || !creditScore || !loanAmount || !loanTerm) return;
    ...
    if (cScoreVal) cScoreVal.textContent = cScore;
    ...
    else if (dtiRatio < 35) score += 10;
    else if (dtiRatio < 50) score -= 15;
    else score -= 35;

    // Clamp score between 8% and 98%
    score = Math.min(Math.max(Math.round(score), 8), 98);
    ...
    // Dynamic Feature Breakdown Highlights based on current inputs
    if (featureList) {
      const features = [];
      if (cScore >= 720) {
        features.push({ icon: 'check', text: `Credit Score > 700 (${cScore}) (+35%)`, favorable: true });
      } else if (cScore >= 640) {
        features.push({ icon: 'check', text: `Fair Credit Score (${cScore}) (+15%)`, favorable: true });
      } else {
        features.push({ icon: 'alert-circle', text: `Low Credit Score (${cScore}) (-25%)`, favorable: false });
      }

      if (dtiRatio <= 25) {
        features.push({ icon: 'check', text: `Debt-to-Income Ratio (${dtiRatio.toFixed(1)}%) < 25% (+25%)`, favorable: true });
      } else if (dtiRatio <= 40) {
        features.push({ icon: 'check', text: `Debt-to-Income Ratio (${dtiRatio.toFixed(1)}%) < 40% (+10%)`, favorable: true });
      } else {
        features.push({ icon: 'alert-circle', text: `High Debt-to-Income Ratio (${dtiRatio.toFixed(1)}%) (-35%)`, favorable: false });
      }

      if (income >= (amount / term) * 3) {
        features.push({ icon: 'check', text: `Sufficient Monthly Cashflow Margin (+15%)`, favorable: true });
      } else {
        features.push({ icon: 'alert-circle', text: `Tight Monthly Income vs Loan Term (-20%)`, favorable: false });
      }

      featureList.innerHTML = features.map(f => `
        <li class="${f.favorable ? 'favorable' : 'risk'}">
          <i data-lucide="${f.icon}"></i> ${f.text}
        </li>
      `).join('');
    }
  ...
  // Attach live event listeners to loan predictor inputs
  [creditScore, applicantIncome, loanAmount, loanTerm].forEach(input => {
    if (input) {
      input.addEventListener('input', () => {
        if (cScoreVal && input === creditScore) cScoreVal.textContent = creditScore.value;
        calculateLoanMl();
      });
      input.addEventListener('change', calculateLoanMl);
    }
  });
  ...
  if (contactForm) {
    ...
    // Live error state removal on typing
